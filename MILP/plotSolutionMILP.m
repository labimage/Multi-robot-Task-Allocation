...
load('solutionMILP.mat');
solution=solutionMILP;
...
for i=1:robotNum
    ...
    for j=1:num
        ...
        tempVec = solution(colIndex+1:colIndex+num,1);
        if ~isempty(find(tempVec == 1, 1))
            rowIndex = find(tempVec == 1);
        else
            rowIndex = 0;
        end
        seq(j,1) = rowIndex;
    ...
    for j=1:num-1
        seq2(j+1,1) = seq(seq2(j,1),1);
    end
    
    RobotTaskSequence{i,1}=seq2;
end
